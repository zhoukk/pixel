...
local c = require "pixel.particle"
local pack = require "spritepack"
local matrix = require "pixel.matrix"
...
function particle_meta.__index:update(dt)
	...
	for _, v in ipairs(self.particles) do
		...
		if visible then
			...
			local active = c.update(v.particle, dt, matrix.new(v.anchor.world_matrix), v.edge)
	...
	if self.group.total_frame > 1 then
		local stay_last = false
		local last_frame = self.group.frame >= self.group.total_frame - 1
		if self.is_active then
			if last_frame then
				if loop_active then
					stay_last = true
					self.group.frame = self.group.total_frame - 1
				else
					self.is_active = false
				end
			end
		...
		if not stay_last and not last_frame then
			self.float_frame = self.float_frame + 1
...
local function new_single(name, anchor)
	...
	if cobj then
		local sprite = pixel.sprite("particle", texid)
...
function particle.new(name, callback)
	...
	local group = pixel.sprite("particle", name)
